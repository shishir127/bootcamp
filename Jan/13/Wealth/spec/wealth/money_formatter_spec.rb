...
describe Wealth::MoneyFormatter do
  ...
  it "returns sum in rupees in the required format" do
    expect(report1.rupee_output(wealth1.amount_in_rupees)).to eq("Sum = Rs. " + wealth1.amount_in_rupees.to_s)
  end

  it "returns sum in paisa in the required format" do
    expect(report1.paisa_output(wealth1.amount_in_paisa)).to eq("Sum = " + wealth1.amount_in_paisa.to_s + " paisa")
  end
