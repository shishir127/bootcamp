...
describe Geometry::Line do
  before(:each) do
    @p_start = Geometry::Point.new(0,0)
    @p_end = Geometry::Point.new(4.0,3.0)
    @line = Geometry::Line.new(@p_start, @p_end)
    @line_inverse = Geometry::Line.new(@p_end, @p_start)
    @line_duplicate = Geometry::Line.new(@p_start, @p_end)
  end
